...
src_install(){
	...
	if use suid ; then
		...
		chmod u+s "${D}"/usr/libexec/singularity/bin/starter-suid
	fi
...
pkg_config(){
	if ! use suid ; then
		sed -i -e "s/allow setuid = yes/allow setuid = no/"  \
		"${D}"/etc/singularity/singularity.conf || die "Unable to sed singularity.conf"
		einfo "Configure singularity to NOT use setuid program."
	fi
